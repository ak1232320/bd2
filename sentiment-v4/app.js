...
let statusArea, reviewDisplay, resultArea, errorArea, analyzeBtn, actionResult;
...
/**
 * Convert model output (label + confidence) to a 0–1 positivity index.
 * NEGATIVE with high confidence → near 0; POSITIVE with high confidence → near 1.
 * @param {string} label - "POSITIVE", "NEGATIVE", or "NEUTRAL".
 * @param {number} score - Model confidence 0–1.
 * @returns {number} Positivity index 0–1.
 */
...
/**
 * Map a positivity index to a business action per the decision matrix.
 * 0.0–0.4 → OFFER_COUPON (churn risk)
 * 0.4–0.7 → REQUEST_FEEDBACK (uncertain)
 * 0.7–1.0 → ASK_REFERRAL (loyal)
 * @param {number} positivity - Normalized score 0–1.
 * @returns {{action: string, message: string, emoji: string, cssClass: string, btnLabel: string}}
 */
function determineBusinessAction(positivity) {
  if (positivity <= 0.4) {
    return {
      action: "OFFER_COUPON",
      message: "We're sorry about your experience! Here's 50% off your next order.",
      emoji: "\u{1F6A8}",
      cssClass: "action-coupon",
      btnLabel: "Claim 50% Off",
    };
  }
  if (positivity < 0.7) {
    return {
      action: "REQUEST_FEEDBACK",
      message: "We'd love to hear more. Help us improve with detailed feedback.",
      emoji: "\u{1F4DD}",
      cssClass: "action-feedback",
      btnLabel: "Take Survey",
    };
  }
  return {
    action: "ASK_REFERRAL",
    message: "Glad you love it! Refer a friend and earn rewards.",
    emoji: "\u{2B50}",
    cssClass: "action-referral",
    btnLabel: "Refer a Friend",
  };
}

/**
 * Display the business action card in #action-result.
 * @param {{action: string, message: string, emoji: string, cssClass: string, btnLabel: string}} data
 * @param {number} positivity - Normalized positivity index for display.
 */
function displayAction(data, positivity) {
  actionResult.className = data.cssClass;
  actionResult.innerHTML =
    `<div style="font-size:2.5rem;margin-bottom:0.5rem">${data.emoji}</div>` +
    `<div style="font-weight:700;font-size:1.1rem;margin-bottom:0.5rem">${data.action}</div>` +
    `<div style="margin-bottom:1rem;opacity:0.95">${data.message}</div>` +
    `<div style="font-size:0.8rem;opacity:0.7;margin-bottom:0.75rem">Positivity Index: ${(positivity * 100).toFixed(1)}%</div>` +
    `<button style="padding:0.6rem 1.5rem;border:2px solid rgba(255,255,255,0.8);background:rgba(255,255,255,0.15);color:#fff;border-radius:8px;font-size:0.95rem;font-weight:600;cursor:pointer">${data.btnLabel}</button>`;
}
...
async function analyzeReview() {
  ...
  try {
    ...
    // Compute business action from normalized positivity index
    const positivity = normalizeScore(label, score);
    const actionData = determineBusinessAction(positivity);
    displayAction(actionData, positivity);

    // Log to Google Sheets with action_taken (non-blocking)
    sendLogSimple({
